...
It's also possible to use a no-code or low-code cloud platforms such as Zapier, Tray.io, or IFTTT. Zapier has a free plan, is the easy to get started, and is specifically designed for system integration. For that reason, the following examples use Zapier. While Zapier makes it easier to get started by building Zaps in a UI, you can also store your Zaps in version control and code them in an IDE using the Developer CLI at https://zapier.com/developer.
